...
| H. 质量评估      | 5    | 生成质量、一致性、幻觉率        |
| I. 单项超长上下文验证 | 4    | 验证超长上下文可用性和边界       |
| J. 回答质量与相关性 | 8    | 回答相关性、乱码检测、无意义回答检测 |
...
| H5  | 响应相关性  | 问答相关性评估                  | 使用不同领域的问题，检查是否包含相关关键词     | P0  |
...
| #   | 测试点              | 测试内容                                   | 评估方法                                    | 优先级 |
|-----|------------------|-----------------------------------------|-----------------------------------------|-----|
| J1-1 | 编程领域相关性        | 验证编程问题的回答相关性                         | 检查回答是否包含编程领域关键词（def/class/函数等）      | P0  |
| J1-2 | 数学领域相关性        | 验证数学问题的回答相关性                         | 检查回答是否包含数学领域关键词（计算/导数/公式等）       | P0  |
| J1-3 | 科学领域相关性        | 验证科学问题的回答相关性                         | 检查回答是否包含科学领域关键词（原子/化学/物理等）       | P1  |
| J2  | 乱码检测            | 检测输出是否为乱码或无效字符                      | 正则检测控制字符、纯符号、异常编码等                 | P0  |
| J3  | 无意义回答检测        | 检测回答是否与问题完全不相关                      | 关键词重叠度分析、trivial response检测          | P1  |
| J4  | 跨领域相关性          | 天气/烹饪等领域相关性验证                       | 跨领域关键词匹配检测                          | P1  |
| J5  | 上下文一致性          | 多轮对话中验证上下文一致性                        | 多轮记忆验证（记住之前提到的内容）                 | P0  |
| J6  | 回答具体性           | 确保回答不是泛泛而谈                            | 长度+关键词详细度检测                         | P2  |
