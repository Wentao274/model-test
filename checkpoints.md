...
| H. 质量评估与回答相关性 | 13   | 生成质量、一致性、幻觉率、回答相关性、乱码检测 |
...
| H2  | 生成一致性  | 相同输入多次生成的稳定性             | seed 固定后比较输出              | P0  |
...
| H6  | 编程领域相关性 | 验证编程问题的回答相关性             | 检查回答是否包含编程领域关键词           | P0  |
| H7  | 数学领域相关性 | 验证数学问题的回答相关性             | 检查回答是否包含数学领域关键词           | P0  |
| H8  | 科学领域相关性 | 验证科学问题的回答相关性             | 检查回答是否包含科学领域关键词           | P1  |
| H9  | 乱码检测   | 检测输出是否为乱码或无效字符           | 正则检测控制字符、纯符号、异常编码等        | P0  |
| H10 | 无意义回答检测 | 检测回答是否与问题完全不相关           | 关键词重叠度分析、trivial response检测 | P1  |
| H11 | 跨领域相关性 | 天气/烹饪等领域相关性验证            | 跨领域关键词匹配检测                | P1  |
| H12 | 上下文一致性 | 多轮对话中验证上下文一致性            | 多轮记忆验证（记住之前提到的内容）         | P0  |
| H13 | 回答具体性  | 确保回答不是泛泛而谈               | 长度+关键词详细度检测               | P2  |
...
F1-F5            稳定性 5 项
G1, G4, G6       API 兼容 3 项
H1, H2, H4, H5, H6, H7, H9, H12  质量评估与回答相关性 8 项
```