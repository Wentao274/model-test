...
TEST_CATEGORIES = {
    ...
    "f_stability": {
        ...
        "tests": [
            ...
            ("F3", "非法参数", "temperature=-1", "invalid_params"),
            ("F4", "特殊字符注入", "SQL/Prompt注入", "special_char_injection"),
            ("F5", "并发稳定性", "200+并发", "concurrent_stability"),
            ("F6", "OOM恢复", "显存耗尽", "oom_recovery"),
            ("F7", "长时间运行", "24小时", "long_running"),
            ("F8", "请求超时处理", "超时断开", "timeout_handling"),
        ],
    },
    "g_api": {
        "name": "G. API兼容性",
        "tests": [
            ("G1", "OpenAI Chat Completions", "/v1/chat/completions 接口兼容", "chat_completions_api"),
            ...
            ("G6", "客户端 SDK 兼容", "Python openai / JS @openai/sdk", "client_sdk_compatibility"),
        ],
    },
    "h_quality": {
        "name": "H. 质量评估",
        "tests": [
            ...
            ("H3", "幻觉率", "事实错误", "hallucination_rate"),
    ...
    "i_long_context": {
        "name": "I. 超长上下文验证",
        "tests": [
            ("I1", "超长上下文（非流式）", "验证超长上下文请求的非流式输出", "super_long_context_create"),
            ("I2", "超长上下文（流式）", "验证超长上下文请求的流式输出", "super_long_context_stream"),
            ("I3", "超长上下文（边界验证）", "使用二分法逼近模型最大上下文长度", "context_boundary_exact_limit"),
            ("I4", "超长上下文（思考模式）", "验证超长上下文下reasoning_content的可用性", "reasoning_content_in_long_context"),
        ],
    },
}
