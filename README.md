...
基于 checkpoints.md 文档设计的大模型推理能力测试框架，覆盖 10 大类共 106 个测试点，用于评估大模型推理服务（OpenAI 兼容 API）的功能完备性与生成质量。
...
| J. clear_thinking 参数行为 | `j_clear_thinking` | 11 | 多轮对话下历史思考的清除/保留、参数正交性、reasoning_content 字段、多 assistant 边界、默认行为探测、行为级验证、last_user 边界 |

> 总计：106 个测试点（P0: 36 / P1: 56 / P2: 14）
...
每个测试用例的优先级在 `base/test_definitions.py` 中配置（合计 106 项）：P0 = 36，P1 = 56，P2 = 14。
