...
class TestLongContextScriptValidation(BaseTest, StreamingTestMixin):
    ...
    @pytest.mark.p1
    @pytest.mark.slow
    def test_super_long_context_create(self, api_client: ModelAPIClient):
        """L1: 超长上下文脚本验证 - 非流式"""
        ...
        try:
            ...
            print(f"Super long context test: prompt_tokens={usage.get('prompt_tokens')}, "
    ...
    @pytest.mark.p1
    @pytest.mark.slow
    def test_super_long_context_stream(self, api_client: ModelAPIClient):
        """L1: 超长上下文脚本验证 - 流式"""
        # 生成超长上下文
        long_prompt = "以下是一篇很长的文章：" + "这是第" + "测试段落。 " * 25000
        ...
        try:
            ...
            print(f"Super long context streaming test: {len(result['chunks'])} chunks, "
    ...
    @pytest.mark.i_long_context
    @pytest.mark.p1
    def test_context_boundary_exact_limit(self, api_client: ModelAPIClient):
        """L1: 上下文边界验证 - 恰好等于限制"""
        # 尝试获取模型最大上下文长度
        try:
            ...
            if max_len > 0:
                ...
                self.assert_response_success(response)
                print(f"Context boundary test: max_len={max_len}, handled successfully")
            else:
                pytest.skip("Model max_model_len not available")
        except Exception as e:
            if "max" in str(e).lower() and "length" in str(e).lower():
                print(f"Context boundary reached: {e}")
            else:
                raise

    @pytest.mark.i_long_context
    @pytest.mark.p1
    def test_reasoning_content_in_long_context(self, api_client: ModelAPIClient):
        ...
        print(f"Long context with thinking: reasoning={len(reasoning) if reasoning else 0} chars, "
